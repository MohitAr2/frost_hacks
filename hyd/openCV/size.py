...
# # TODO: Use a machine learning model to identify the person and their clothes in the image
# # Once you've identified the clothes, you can calculate the bounding rectangle
# x, y, w, h = cv2.boundingRect(clothes)
# # Save the width and height to a JSON file
# with open('sizes.json', 'w') as f:
#     json.dump({'shirt_width': w, 'shirt_height': h}, f)
#make wsmtg that readws size of human shirt size and pant size thats all
import cv2
import colorsys
import imghdr
import os
...
class User:
    def __init__(self, name, shirt_size, pant_size):
        self.name = name
        self.shirt_size = shirt_size
        self.pant_size = pant_size
    def display_sizes(self):
        print(f"User {self.name} has shirt size {self.shirt_size} and pant size {self.pant_size}")
def _main():
    # Paths
